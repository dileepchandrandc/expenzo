...
public class ExpenseService {
    ...
    public PaginatedResponse<ExpenseDto> getExpenses(Integer userId, Integer year, Integer month, int page, int size) {
        LocalDateTime[] startEndDates = DateTimeUtils.getBoundaryDateTimes(year, month);
        List<ExpenseDto> expenses =  expenseRepository.getExpense(userId, startEndDates[0], startEndDates[1], page * size, size + 1);
