...
const bankId = ref('');
const accountType = ref<BankAccountType>('SAVINGS');
  ...
  try {
    ...
    if (err && typeof err === 'object' && 'response' in err) {
      ...
      if (typeof data === 'object' && data !== null && !('error' in data)) {
        ...
        error.value = (data as { error?: string })?.error || 'Failed to add bank account';
      }
    } else {
      error.value = 'Failed to add bank account. Please try again.';
    }
  } finally {
